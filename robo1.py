...
def get_state(lidar, state_space):
	threshold  = min(lidar)+0.5 if min(lidar)<1 else 0
	#print(threshold)
	x1 = 0 #lado direito
	x2 = 0 #lado esquerdo
...
global ciclos
try:
	etapas = np.load('etapas.npy').tolist()
	medias_recompensas = np.load('medias_recompensas.npy').tolist()
	tempos = np.load('tempos.npy').tolist()
	ciclos = np.load('ciclos.npy').tolist()
	recompensas = []
	recompensas.append(medias_recompensas[-1])
except:
	etapas = []
	medias_recompensas = []
	tempos = []
...
while not rospy.is_shutdown():
	...
	while True:
		...
		while True:
			...
			msgScan = rospy.wait_for_message('/robot_0/base_scan', LaserScan)
			...
			if min(lidar)<0.5 and flag1==0:
				...
				while min(lidar)<0.5:
					...
					if crash==1 or r==-100:
						...
						tempos.append(deltaT)
						etapas.append(etapa)
						medias_recompensas.append(np.mean(recompensas))
						ciclos.append(ciclo)
						ciclo = 0
						#print(ciclos)
						np.save('etapa.npy', etapa)
						np.save('etapas.npy', etapas)
						np.save('tempos.npy', tempos)
						np.save('medias_recompensas.npy', medias_recompensas)
						np.save('ciclos.npy', ciclos)
						#ciclos = []
		...
		ciclo+=1
		#if posi_x==5.94 and posi_y==-9.07:
		#	ciclo-=1
		#print(ciclo)
		tempo_agora = time.time()
		l = 0
